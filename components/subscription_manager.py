...
def render_subscription_manager():
    ...
    with tab2:
        render_plan_management()

    with tab3:
        render_user_subscriptions()

    with tab4:
        render_revenue_analytics()

    with tab5:
        render_payment_settings()
...
def render_user_subscriptions():
    ...
    db = SessionLocal()
    try:
        query = db.query(User, Subscription).join(Subscription)
        if search:
            query = query.filter(
                or_(
